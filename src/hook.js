...
const ENABLE_LOCAL_VIP = ['true', 'cvip', 'svip'].includes((process.env.ENABLE_LOCAL_VIP || '').toLowerCase());
const ENABLE_LOCAL_SVIP = (process.env.ENABLE_LOCAL_VIP || '').toLowerCase() === 'svip';
const LOCAL_VIP_UID = (process.env.LOCAL_VIP_UID || '').split(',').map((str) => parseInt(str)).filter((num) => !Number.isNaN(num));
            ...
            .then((body) => {
                ...
                if (body) {
                    ...
                    if (
                        ...
                        HIRES &&
                        (netease.path === "/api/song/enhance/player/url" ||
                ...
                const inject = (key, value) => {
                    if (typeof value === "object" && value != null) {
                        ...
                        if (
                            "fl" in value &&
                            "maxbr" in value &&
                            value["fl"] < value["maxbr"]
                        )
                            value["fl"] = value["maxbr"];
                        if (HIRES) {
                            if (
                                "dlLevel" in value &&
                                "downloadMaxBrLevel" in value &&
                                value["downloadMaxBrLevel"] !== "none"
                            )
                                value["dlLevel"] = value["downloadMaxBrLevel"];
                            if (
                                "plLevel" in value &&
                                "playMaxBrLevel" in value &&
                                value["playMaxBrLevel"] !== "none"
                            )
                                value["plLevel"] = value["playMaxBrLevel"];
                            if (
                                "flLevel" in value &&
                                "maxBrLevel" in value &&
                                value["maxBrLevel"] !== "none"
                            )
                                value["flLevel"] = value["maxBrLevel"];
                        }
...
const pretendDownload = (ctx) => {
    ...
    if (netease.forward) {
        ...
        const { ids, level, e_r, header } = netease.param;
        netease.param = {
            id: JSON.parse(ids)[0],
            level,
            e_r,
            ...
            return match(item.id)
                .then((song) => {
                    ...
                    item.url = global.endpoint
                        ? `${global.endpoint}/package/${crypto.base64.encode(
                              song.url
                          )}/${item.id}.${item.type}`
                        : song.url;
