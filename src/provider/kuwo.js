...
const encryptKey = "Hm_Iuvt_cdb524f42f0cer9b268e4v7y734w5esq24";
var weight = 0;
	...
	return request("GET", `http://kuwo.cn/search/list?key=${keyword}`)
		.then((response) =>
			response.headers["set-cookie"]
				.find((line) => line.includes(encryptKey))
				.replace(/;.*/, "")
				.split("=")
				.pop()
		)
		.then((data) =>
			request("GET", url, {
				...
				Cookie: `${encryptKey}=${data}`,
				Secret: crypto.kuwoapi.SecretCalc(data, encryptKey),
			})
		)
