...
const SecretCalc = (buffer, key) => {
	if (null == key || key.length <= 0) {
		console.log(
			"Key is null or empty. Please choose a password with 1 or more characters."
		);
		return null;
	}
	var n = "";
	...
	if (o < 2) {
		console.log(
			"Algorithm cannot find a suitable hash. Please choose a password with 8 or more characters."
		);
		return null;
	}
	...
	for (i = 0; i < buffer.length; i++) {
		f +=
			(h = parseInt(buffer.charCodeAt(i) ^ Math.floor((n / c) * 255))) <
			16
				? "0" + h.toString(16)
				: h.toString(16);
		n = (o * n + l) % c;
...
module.exports = { encrypt, decrypt, encryptQuery, SecretCalc };
